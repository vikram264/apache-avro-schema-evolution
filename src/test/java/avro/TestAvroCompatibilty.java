...
import example.CustomerV1;
import example.CustomerV2;
import example.EvolvedPerson;
...
public class TestAvroCompatibilty {
    ...
    public void testSchemaEvolution() throws Exception {
        ...
        DataFileWriter<Person> fileWriter = new DataFileWriter<>(writer);
        fileWriter.create(person.getSchema(), new File("person.avro"));
        fileWriter.append(person);
        fileWriter.close();

        //Test Backward Compatibilty
        DatumReader<EvolvedPerson> reader = new SpecificDatumReader<>(EvolvedPerson.class);
        DataFileReader<EvolvedPerson> fileReader = new DataFileReader<>(new File("person.avro"), reader);
        EvolvedPerson evolvedPerson = fileReader.next();
        fileReader.close();

        System.out.println(evolvedPerson);
    }

    @Test
    public void testBackwardCompatibility() throws IOException {
        CustomerV1 customerV1 = CustomerV1.newBuilder().setAge(34).setAutomatedEmail(false).setFirstName("John").setLastName("Doe").setHeight(100.0f).setWeight(56.6f).build();
